...
current_line = []  # 현재 그리고 있는 선의 점들을 저장함
current_polygon = []  # 현재 그리고 있는 다각형의 점들을 저장함

# 마우스 이벤트를 처리하기 위한 콜백 함수
def draw_line(event, x, y, flags, param):
    global drawn_lines, line_color, current_line
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        current_line.append((x, y))  # 현재 점을 선에 추가

    elif event == cv2.EVENT_LBUTTONUP:
        if len(current_line) == 2:  # 완료된 선 확인
            drawn_lines.append(current_line.copy())  # 선을 그린 선 목록에 추가
            cv2.line(frame, current_line[0], current_line[1], line_color, 2)
            current_line.clear()

# 스페이스바를 누를 때까지 점을 이어 면 영역으로 처리
def connect_points(event, x, y, flags, param):
    global drawn_lines, current_polygon

    if event == cv2.EVENT_LBUTTONDOWN and len(drawn_lines) > 0:
        if len(current_polygon) == 0:
            current_polygon.append((x, y))  # 첫 번째 점 추가
        else:
            cv2.line(frame, current_polygon[-1], (x, y), line_color, 2)
            current_polygon.append((x, y))  # 다음 점 추가
# 선과 점이 교차하는지 확인하는 함수
...
while True:
    ...
    if len(current_polygon) > 1:
        cv2.polylines(frame, [np.array(current_polygon)], True, line_color, 2)
    ...
    if key == ord("q"):
        ...
        if len(current_polygon) > 2:
            drawn_lines.append(current_polygon.copy())
